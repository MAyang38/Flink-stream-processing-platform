...
//public  class CustomProcessWindowFunction extends ProcessAllWindowFunction<Tuple3<Double, Double, String>, Object, TimeWindow>{
public  class CustomProcessWindowFunction extends ProcessWindowFunction<ClusterReading,String, Tuple, TimeWindow> {
   ///DPC算法
...
//            double rhomax =rho.get(new NDIndex("{}",centersmax)).toDoubleArray()[0];
    ...
    public void process(Tuple tuple, Context context, Iterable<ClusterReading> iterable, Collector<String> collector) throws Exception {
        ...
        int count = 0;
        //增强for循环遍历全窗口所有数据
        for (ClusterReading ele : iterable) {
            count++;
        }
